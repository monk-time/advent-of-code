...
(f => { console.time('main'); f(); console.timeEnd('main'); })(() => { // eslint-disable-line
    ...
    const hash = (area, elev) => sortByMG(area) // 4 floors * 8 items per floor = 32 bits
        .reduce((h, [m, g]) => (h << 4) + (m << 2) + g, elev); // eslint-disable-line no-bitwise
    ...
    const combs = function* ([pairF, mF, gF], area, floor) {
        ...
        for (let i = 0; i < area.length; i++) {
            ...
            if (mHere) yield replace(area, mF(area[i]), i); // take all microships
            if (gHere) yield replace(area, gF(area[i]), i); // take all generators
        }
    };
    ...
    const bfs = (area, floor) => {
        ...
        while (queue.length) {
            ...
            if (floor === 3 && emptyBelow(area, 3)) break;
            for (const { area: areaNew, floor: floorNew } of safeMoves(area, floor)) {
