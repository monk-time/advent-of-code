...

{
    ...
    const reachable = (ducts, i, j) => [[i + 1, j], [i - 1, j], [i, j - 1], [i, j + 1]].filter(
        ([y, x]) => ducts[y][x] !== false);
    const shortestDist = (ducts, [i0, j0], [iEnd, jEnd]) => { // bfs
        ...
        while (queue.length) {
            ...
            for (let [iNext, jNext] of reachable(ducts, i, j)) {
    ...
    for (let i = 0; i < stops.length; i++) {
        for (let j = i; j < stops.length; j++) {
            minDists[i][j] = minDists[j][i] = shortestDist(ducts, stops[i], stops[j]);
        }
    }
