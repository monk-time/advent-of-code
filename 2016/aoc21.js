...

{
    ...
    const op = {
        ...
        reverse: (x, y) => a => [...a.slice(0, +x), ...a.slice(+x, +y + 1).reverse(), ...a.slice(+y + 1)],
        move: (x, y) => a => {
            let a2 = [...a];
    ...
    const opR = Object.assign({}, op, {
        ...
        rotate: x => a => {
            ...
            for (let i = 0; i < a.length; i++) {
                ...
                if (op.rotate(x)(res).join('') === a.join('')) {
                    return res;
                }
            }
        }
    ...
    const exec = (ops, list, pass) => list.reduce(
        (a, s) => ops[parseType(s)](...parseArgs(s))(a), [...pass]).join('');
