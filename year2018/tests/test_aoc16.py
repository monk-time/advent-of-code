...
from year2018.aoc16 import Tuple4, execute, matches, opcodes, parse, solve
...
@pytest.mark.parametrize('instr, reg, reg_after', samples)
def test_execute(
    instr: tuple[str, int, int, int], reg: Tuple4, reg_after: Tuple4
):
    fixed_instr = (opcodes.index(instr[0]), *instr[1:])
    assert execute(fixed_instr, reg) == reg_after
